        ...
        class Arguments
          ...
          def match!(args:)
            ...
            with.each_with_index do |s, i|
              ...
              # Sometimes signature contains extra elements that can be nil. In that case we allow it to not be passed.
          ...
          def bad_arg_error(arg_signature:, index:, arg:)
            raise "bad arg #{index}, method #{method_name}: #{arg}/#{arg.class} #{arg_signature}"
          end
        end
      end
    end
